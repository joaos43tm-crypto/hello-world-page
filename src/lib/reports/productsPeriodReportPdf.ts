...
export type ProductsPeriodReportRow = {
  productId: string;
  name: string;
  qty: number;
  revenue: number;
...
export type ProductsPeriodReportInput = {
  storeName: string;
  store?: {
    address?: string | null;
...
export async function generateProductsPeriodReportPdf(input: ProductsPeriodReportInput) {
  ...
  drawText("Relatório de Produtos", { bold: true, size: 14 });
  ...
  drawText(`Itens vendidos: ${input.summary.itemsTotal}`, { size: 11 });
  drawText(`Faturamento produtos: ${money(input.summary.revenueTotal)}`, { size: 11 });
  drawText(`Custo total estimado: ${money(input.summary.costTotal)}`, { size: 11 });
  drawText(`Lucro bruto estimado: ${money(input.summary.profitTotal)}`, { size: 11 });
  drawText(`Margem média (ponderada): ${pct(input.summary.marginPct)}`, { size: 11 });
  y -= 6;
  drawLine();

  // ===== Table =====
  drawText("Produtos mais vendidos (por quantidade)", { bold: true, size: 12 });

  const colName = 260;
  const colQty = 55;
  const colRevenue = 95;
  const colCostUnit = 80;
  const colCostTotal = 95;
  const colProfit = 85;
  const colMargin = Math.max(70, contentWidth - (colName + colQty + colRevenue + colCostUnit + colCostTotal + colProfit));

  drawRow(
    [
      { text: "Produto", width: colName },
      { text: "Qtd", width: colQty, align: "right" },
      { text: "Faturamento", width: colRevenue, align: "right" },
      { text: "Custo (un)", width: colCostUnit, align: "right" },
      { text: "Custo total", width: colCostTotal, align: "right" },
      { text: "Lucro", width: colProfit, align: "right" },
      { text: "Margem", width: colMargin, align: "right" },
    ],
    { bold: true }
  );
  drawLine();
  ...
  for (const r of input.rows) {
    const nameLines = wrapText(r.name, font, 10, colName);
    for (let i = 0; i < nameLines.length; i++) {
      if (i === 0) {
        drawRow([
          { text: nameLines[i], width: colName },
          { text: String(r.qty), width: colQty, align: "right" },
          { text: money(r.revenue), width: colRevenue, align: "right" },
          { text: money(r.costUnit), width: colCostUnit, align: "right" },
          { text: money(r.costTotal), width: colCostTotal, align: "right" },
          { text: money(r.profit), width: colProfit, align: "right" },
          { text: pct(r.marginPct), width: colMargin, align: "right" },
        ]);
      } else {
        drawRow([
          { text: nameLines[i], width: colName },
          { text: "", width: colQty },
          { text: "", width: colRevenue },
          { text: "", width: colCostUnit },
          { text: "", width: colCostTotal },
          { text: "", width: colProfit },
          { text: "", width: colMargin },
        ]);
      }
    }
  }
